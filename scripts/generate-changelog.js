...
try {
  ...
  const output = execSync('npx conventional-changelog --preset conventionalcommits --release-count 1 --stdout 2>/dev/null', {
    cwd: process.cwd(),
    encoding: 'utf-8',
  ...
  if (output && output.split('\n').filter(line => line.trim() && !line.startsWith('#')).length > 0) {
    ...
    const fallbackOutput = execSync('git log --oneline -5 $(git describe --tags --abbrev=0)..HEAD 2>/dev/null | head -20 || git log --oneline -5', {
      cwd: process.cwd(),
      encoding: 'utf-8',
    }).trim();
